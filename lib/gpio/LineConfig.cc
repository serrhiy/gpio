...
namespace gpio {
  ...
  bool LineConfig::isValid() const { return line_config != nullptr; }
  ...
  LineConfig::LineConfig(): line_config{ gpiod_line_config_new() } {
    if (!isValid()) {
  ...
  void LineConfig::addLineSettings(
    ...
    if (result == -1) {
      throw std::runtime_error{ "addLineSettings error: " +
  ...
  LineSettings LineConfig::getLineSettings(const unsigned offset) const {
    ...
    gpiod_line_settings* line_settings =
      gpiod_line_config_get_line_settings(line_config, offset);
  ...
  void LineConfig::setOutputValues(const std::vector<LineValue>& values) const {
    ...
    const auto view = values | transform([](const auto& value) {
                        return line_value_reverse_map.at(value);
                      });
    std::vector<gpiod_line_value> data(view.begin(), view.end());

    int result = gpiod_line_config_set_output_values(
      line_config,
      data.data(),
      data.size()
    );
    if (result == -1) {
      throw std::runtime_error{ "setOutputValues error: " +
                                std::to_string(result) };
    }
  }
